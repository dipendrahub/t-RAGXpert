...
# ── Load Upgraded7 KG — used ONLY for role diversity measurement ───────────────
# The baseline ranking itself is still citation-based (unchanged).
# We load the KG solely to measure role diversity of the citation-ranked output,
# enabling a fair comparison with the role-diverse system.
print("Loading Upgraded KG for role diversity measurement...")
kg_with_roles = Graph()
kg_with_roles.parse(
...
def main():
    ...
    print("Starting Baseline Expert Search Process...")
    print("Ranking method: Citation-based (baseline)")
    ...
    # ── RAG context KG path (original — unchanged for baseline) ───────────────
    rdf_file = "/Utilisateurs/dsharm01/repo/KG-Method/knowledge_graph_inferred_full_dataset.ttl"
    ...
    ndcg5_scores          = []
    ndcg10_scores         = []
    role_diversity_scores = []   # measures seniority bias of citation ranking
    ...
    for prompt_keyword in tqdm(prompt_keywords, desc="Processing Queries.."):
        ...
        # ── BASELINE: citation-based ranking (unchanged) ───────────────────────
        ranked_expert_ids = filter_data.rank_experts_by_citations(expert_ids, authors_df)
        predicted_ids     = ranked_expert_ids

        # ── Measure role diversity of citation-ranked output ───────────────────
        # This quantifies seniority bias introduced by citation ranking.
        # Low score = dominated by supervising/senior authors (expected for baseline).
        diversity_score = filter_data.compute_role_diversity_score(
            predicted_ids, kg_with_roles, top_k=10
        )
        print(f"Role diversity score (top-10): {diversity_score:.4f}")

        # ── Get RAG contexts (original KG — unchanged for baseline) ───────────
        ...
        # ── LLM author reranking (unchanged) ──────────────────────────────────
