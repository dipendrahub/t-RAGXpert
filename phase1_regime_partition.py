...
authors_df = authors_df.merge(recency_df[["author_id", "recency_ratio", "tensor_total"]],
                               on="author_id", how="left")
authors_df["recency_ratio"] = authors_df["recency_ratio"].fillna(0)
authors_df["tensor_total"]  = authors_df["tensor_total"].fillna(0).astype(int)

# ── Year coverage flag ────────────────────────────────────────────────────────
# Partial: tensor covers <20% of total_pubs -> likely missing year data
coverage_ratio = authors_df["tensor_total"] / authors_df["total_pubs"].replace(0, 1)
authors_df["year_coverage"] = "full"
authors_df.loc[coverage_ratio < 0.20, "year_coverage"] = "partial"
authors_df.loc[authors_df["tensor_total"] == 0, "year_coverage"] = "none"

partial_count = (authors_df["year_coverage"] == "partial").sum()
none_count    = (authors_df["year_coverage"] == "none").sum()
print(f"  -> Year coverage flags:")
print(f"       full    : {(authors_df['year_coverage'] == 'full').sum():,}")
print(f"       partial : {partial_count:,}  (tensor covers <20% of known pubs)")
print(f"       none    : {none_count:,}  (no papers in tensor)")
...
print(f"\nYear coverage breakdown by regime:")
for regime in ["A", "B"]:
    sub = authors_df[authors_df["regime"] == regime]
    print(f"  Regime {regime}:")
    for flag in ["full", "partial", "none"]:
        n = (sub["year_coverage"] == flag).sum()
        print(f"    {flag:<8} : {n:,}  ({n/len(sub)*100:.1f}%)")

print(f"\nTop partial-coverage authors in Regime A (high pubs, low tensor coverage):")
partial_a = (authors_df[(authors_df["regime"] == "A") &
                         (authors_df["year_coverage"] == "partial")]
             .sort_values("total_pubs", ascending=False)
             .head(10))
print(f"\n  {'Name':<30} {'Pubs':>6} {'TensorPubs':>11} {'Span':>5} {'Coverage':>9}")
print("  " + "-" * 68)
for _, row in partial_a.iterrows():
    cov = row["tensor_total"] / max(row["total_pubs"], 1) * 100
    print(f"  {row['name']:<30} {row['total_pubs']:>6} "
          f"{row['tensor_total']:>11} {row['career_span']:>5} {cov:>8.1f}%")